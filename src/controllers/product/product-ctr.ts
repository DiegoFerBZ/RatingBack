...
class ProductController {
  public async registerProduct(
    ...
    const newProduct = await producSvs.registerNewProduct(
      name,
      description,
      url_img,
      review,
    ...
    res
      .status(201)
      .json({ message: "Producto registrado con éxito.", product: productDTO });
  }

  public async getProductOrProducts(
    req: Request,
    res: Response,
    next: NextFunction
  ): Promise<void> {
  ...
  public async makeAComment(
    ...
    const newComment: Comment = await producSvs.makeAComment(
      content,
      user_id,
      product_id
    );
  ...
  public async getCommentsByProduct(
    req: Request,
    res: Response,
    next: NextFunction
  ): Promise<void> {
